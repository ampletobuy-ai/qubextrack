#!/usr/bin/env python3
"""Build restaurant competitor comparison HTML from RESTAURANT_COMPETITOR_COMPARISON.md (section 2 only)."""
import re
from pathlib import Path

ROOT = Path(__file__).resolve().parent
MD_PATH = ROOT / "RESTAURANT_COMPETITOR_COMPARISON.md"
PARTIAL_PATH = ROOT / "assets/partials/restaurant-competitor-comparison.html"
PAGE_PATH = ROOT / "restaurant-pos-software.html"
...
def inject_into_page(html: str) -> None:
    ...
    start_marker = '<template id="restaurant-competitor-comparison-template">'
    end_marker = "</template>"
    modal_marker = 'id="restaurant-competitor-comparison"'
    if start_marker in page:
        ...
        insert_block = f'''
        <!-- Competitor comparison modal (restaurant) -->
        <template id="restaurant-competitor-comparison-template">
{html}
        </template>
        <div class="trackpos-modal trackpos-modal--wide" id="restaurant-competitor-comparison" aria-hidden="true" data-trackpos-modal-template="restaurant-competitor-comparison-template">
          <div class="trackpos-modal__overlay" aria-hidden="true"></div>
          <div class="trackpos-modal__dialog" role="dialog" aria-modal="true" aria-labelledby="restaurant-competitor-comparison-title">
            <div class="trackpos-modal__header">
              <h4 class="trackpos-modal__title" id="restaurant-competitor-comparison-title">Full feature comparison</h4>
...
'''
        ...
        fallback = '        <section id="pricing" class="wrapper !bg-[#ffffff] trackpos-jewellery-pricing">'
        if anchor in page:
            new_page = page.replace(anchor, "\n        </section>\n\n" + insert_block + "\n\n        <div class=\"flex flex-wrap mx-[-15px] !mt-12\">", 1)
        elif fallback in page:
            new_page = page.replace(fallback, insert_block + "\n" + fallback, 1)
        else:
            raise SystemExit("Modal insert anchor not found in restaurant-pos-software.html")
